...
#
  ...
  if [ $? -eq 0 ] ; then
    ...
    /usr/bin/sacctmgr -i  add  user Name=$username  Partition=$PART  QOS=$QOS Account=$ACCOUNT AdminLevel=None
  fi
}
...
check_account() {
  ...
  if [ $? -eq 0 ] ; then
    ...
    if [[ $first_2_char == "pc" ]]; then
      ...
      if [[ -z $PC_SU ]]; then
        /usr/bin/sacctmgr modify account where name=$ACCOUNT set GrpTRESMins="cpu=18000000" qos="lr_debug,lr_normal"
      else
        /usr/bin/sacctmgr modify account where name=$ACCOUNT set GrpTRESMins="cpu=$PC_SU" qos="lr_debug,lr_normal"
      fi
    else
      /usr/bin/sacctmgr create account name=$ACCOUNT Description="$ACCOUNT cluster" Org="$ACCOUNT"
    fi
  fi
}
  ...
  case $first_2_char in
    ...
    ac|scs|ld|pc)
      ...
      for LR_PART in $MANYCORE_LRC; do
        /usr/bin/sacctmgr -i modify user where name=$username account=$ACCOUNT partition=$LR_PART set qos=lr_normal,lr_debug
      done
      if [ "${ACCOUNT}" = "pc_heptheory" ]; then
        check_account $ACCOUNT
        /usr/bin/sacctmgr -i add user $username  account=$ACCOUNT qos=lr_interactive partition=lr3_htc   # for the htc testbed
      fi
      ;;
    ...
    lr)
      if [ "${ACCOUNT}" == "lr_cumulus" ]; then
        check_account $ACCOUNT
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=condo_cumulus
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=condo_cumulus_lr6   # condo for David Romps
      elif [ "${ACCOUNT}" = "lr_mp" ] ; then
        check_account $ACCOUNT
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=condo_mp_lr2   # Condo for Kristin
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=cf1 qos=condo_mp_cf1   # Condo for Kristin
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=cf1-hp qos=condo_mp_cf1   # Condo for Kristin
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=es1 qos=condo_mp_es1   # Condo for Kristin
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=condo_mp_lr6   # condo for Sam Blau
      ## Condo for Krinith
      elif [ "${ACCOUNT}" = "lr_chandra" ] ; then
        check_account $ACCOUNT
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=es1 qos=condo_chandra_es1
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=csd_lr6_192 qos=condo_chandra_lr6
      ## Condo for Teresa Head-Gordon
      elif [ "${ACCOUNT}" = "lr_ninjaone" ] ; then
        check_account $ACCOUNT
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=cm2 qos=condo_ninjaone_cm2
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=csd_lr6_192 qos=condo_ninjaone
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=csd_lr6_share qos=condo_ninjaone_share
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=es1 qos=condo_ninjaone_es1

      elif [ "${ACCOUNT}" = "lr_amos" ] ; then
        check_account $ACCOUNT
        ## Robert  Lucchese/McCurdy 12 nodes
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr7 qos=condo_amos7_lr7,lr_lowprio
        ## Robert Lucchese  28 nodes
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=csd_lr6_192 qos=condo_amos,lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr5 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr3 qos=lr_lowprio

      elif [ "${ACCOUNT}" = "lr_essdata" ] ; then
        check_account $ACCOUNT
        ## Shreyas Cholia 8 nodes
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=condo_essdata_lr6,lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr5 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr3 qos=lr_lowprio
        ...
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr7 qos=condo_mhg_lr7,lr_lowprio
        ## Martin Head Gordon  18 nodes
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=csd_lr6_192 qos=condo_mhg2,lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr5 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr3 qos=lr_lowprio
        ...
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr7 qos=condo_rncstar_lr7,lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr6 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr5 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr4 qos=lr_lowprio
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr3 qos=lr_lowprio
        ...
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=es1 qos=condo_nanotheory_es1,es_lowprio
        ## David Prendergast 4 nodes
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=lr3 qos=condo_nanotheory,lr_lowprio
        ...
        for lowprio_partition in $lowprio_partitions ; do
          /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=${lowprio_partition} qos=lr_lowprio
        done
        /usr/bin/sacctmgr -i modify user $username account=$ACCOUNT partition=lr7 qos+=condo_geop_lr7

        # Nori Nakata 1 ea. Gigabyte GPU node, 4 x A40 GPU; po_number=7734064; req#1000478254; ES1 Condo
        /usr/bin/sacctmgr -i create user $username account=$ACCOUNT partition=es1 qos=condo_geop_es1,es_lowprio
